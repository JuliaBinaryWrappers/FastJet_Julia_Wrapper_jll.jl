...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import FastJet_Julia_Wrapper_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function dev_jll()
        ...
        @info("FastJet_Julia_Wrapper_ll dev'ed out to /depot/dev/FastJet_Julia_Wrapper_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("33ff6b16-4e4c-5b96-9396-f34816859298"))
platforms = [Pkg.Artifacts.unpack_platform(e, "FastJet_Julia_Wrapper", artifacts_toml) for e in artifacts["FastJet_Julia_Wrapper"]]
...
if best_platform === nothing
    @debug("Unable to load FastJet_Julia_Wrapper; unsupported platform $(triplet(platform_key_abi()))")
else
